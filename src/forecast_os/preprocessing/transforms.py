...
handles forecast frames: it applies the inverse to every value column present
— ``y``, ``yhat``, and interval columns matching ``lo-*``/``hi-*`` or
``*-lo-*``/``*-hi-*`` — leaving all other columns untouched.
"""
...
def _value_columns(df: pd.DataFrame) -> list[str]:
    """Columns the inverse applies to: y, yhat, and interval columns."""
    return [
...
class BaseTransform:
    ...
    def _apply_per_series(self, df: pd.DataFrame, func) -> pd.DataFrame:
...
class StandardScaler(BaseTransform):
    ...
    def inverse_transform(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        return self._apply_per_series(df, _unscale)
...
class LogTransform(BaseTransform):
    ...
    def inverse_transform(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        return self._apply_per_series(df, _exp)
...
class Differencer(BaseTransform):
    ...
    two kinds of frames: the transformed training panel itself (rows within
    the training range) and frames whose ``ds`` all lie **after** the training
    series (forecast continuations). Arbitrary slices are not invertible.
    """
    ...
    def inverse_transform(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for uid, idx in out.groupby(ID_COL, sort=False).indices.items():
            ...
            ds = out[TIME_COL].to_numpy()[idx]
            continuation = bool((ds > np.asarray(last_ds)).all())
            for c in cols:
                vals = out[c].to_numpy(dtype=float)[idx]
